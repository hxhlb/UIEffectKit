...
public enum BreakGlassTransition {
    #if canImport(UIKit)
        @discardableResult
        public static func performInWindow(
            for target: UIView,
            fractureCount: Int = 96,
            onFirstFrame: (() -> Void)? = nil,
            completion: (() -> Void)? = nil
        ) -> Bool {
            guard let window = target.window else { completion?(); return false }
            guard MetalCheck.isMetalSupported else { completion?(); return false }

            let snapshot = target.createViewSnapshot()
            guard let image = snapshot.cgImage else { completion?(); return false }
            ...
            let transitionView = BreakGlassView(frame: overlay.bounds)
            overlay.addSubview(transitionView)

            transitionView.begin(
                with: image,
                targetFrame: frameInWindow,
                fractureCount: fractureCount,
                onFirstFrameRendered: { onFirstFrame?() },
                onComplete: {
                    completion?()
        ...
        public static func performInWindow(
            ...
            guard let window = target.window, let content = window.contentView else { completion?(); return false }
            guard MetalCheck.isMetalSupported else { completion?(); return false }

            guard let image = target.createViewSnapshot() else { completion?(); return false }
